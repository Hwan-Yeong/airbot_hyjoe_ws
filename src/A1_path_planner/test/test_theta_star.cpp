...
public:
  ...
  bool ulosCheck(const int &x0, const int &y0, const int &x1, const int &y1,
                 double &sl_cost) {
    return losCheck(x0, y0, x1, y1, sl_cost);
  }

  bool uwithinLimits(const int &cx, const int &cy) {
    return withinLimits(cx, cy);
  }
  ...
  void uaddIndex(const int &cx, const int &cy) {
    addIndex(cx, cy, &nodes_data_[0]);
  }

  tree_node *ugetIndex(const int &cx, const int &cy) {
    return getIndex(cx, cy);
  }
...
TEST(ThetaStarTest, test_theta_star) {
  ...
  n.x = 25;
  EXPECT_FALSE(
      planner_->uisGoal(n)); // only y coordinate matches with that of goal
  n.x = g.x;
  n.y = 20;
  EXPECT_FALSE(
      planner_->uisGoal(n)); // only x coordinate matches with that of goal
  n.x = 30;
  EXPECT_FALSE(planner_->uisGoal(
      n)); // both (x, y) are different from the goal coordinate

  /// Check if the isSafe functions work properly
  EXPECT_TRUE(planner_->isSafe(5, 5)); // cost at this point is 0
  EXPECT_FALSE(
      planner_->isSafe(10, 10)); // cost at this point is 253 (>LETHAL_COST)
...
TEST(ThetaStarPlanner, test_theta_star_planner) {
  ...
  start.pose.orientation.w = 1.0;
  goal = start;
  auto planner_2d = std::make_unique<A1_path_planner::PathPlanner>();
...
TEST(ThetaStarPlanner, test_theta_star_reconfigure) {
  rclcpp_lifecycle::LifecycleNode::SharedPtr life_node =
      std::make_shared<rclcpp_lifecycle::LifecycleNode>("A1PathPlannerTest");

  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros =
      std::make_shared<nav2_costmap_2d::Costmap2DROS>("global_costmap");
  costmap_ros->on_configure(rclcpp_lifecycle::State());

  auto planner = std::make_unique<A1_path_planner::PathPlanner>();
  try {
  ...
  auto rec_param = std::make_shared<rclcpp::AsyncParametersClient>(
      ...
      life_node->get_node_topics_interface(),
  ...
  rclcpp::spin_until_future_complete(life_node->get_node_base_interface(),
                                     results);
  ...
  EXPECT_EQ(
      life_node->get_parameter("test.use_final_approach_orientation").as_bool(),
      false);
