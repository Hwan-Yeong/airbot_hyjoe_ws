...
#ifndef A1_PATH_PLANNER__THETA_STAR_HPP_
#define A1_PATH_PLANNER__THETA_STAR_HPP_
...
class ThetaStar
{
   ...
   protected:
     ...
     * @param cost denotes the total straight line traversal cost; it adds the traversal cost for the node (cx, cy) at
     * every instance; it is also being returned
    ...
    /**
     * @brief initialises the node_position_ vector by storing -1 as index for all points(x, y) within the limits of the
     * map
     * @param size_inc is used to increase the number of elements in node_position_ in case the size of the map
     * increases
     */
    ...
    /**
     * @brief this function depending on the size of the nodes_data_ vector allots space to store the data for a node(x,
     * y)
...
#endif  //  A1_PATH_PLANNER__THETA_STAR_HPP_
