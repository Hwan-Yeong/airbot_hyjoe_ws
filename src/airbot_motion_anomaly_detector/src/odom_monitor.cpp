...
void OdomMonitor::data_timer_callback() {
    ...
    if (odom_diff.seconds() > 0.05) {
        ...
        if(m_no_odom_data_msg_latch < 5) {
            RCLCPP_WARN(this->get_logger(), "No odometry data received for %.2f seconds.[%d]", odom_diff.seconds(), m_no_odom_data_msg_latch);
        ...
        if(m_no_odom_data_msg_latch != 0){
            rclcpp::Duration odom_normal_duration = last_odom_time_ - no_odom_detected_time_;
            RCLCPP_WARN(this->get_logger(), "odometry data received ok after %.2f seconds.", odom_normal_duration.seconds());            
        }
    ...
    if (imu_diff.seconds() > 0.05) {
        ...
        if(m_no_imu_data_msg_latch < 5) {
            RCLCPP_WARN(this->get_logger(), "No IMU data received for %.2f seconds.[%d]", imu_diff.seconds(), m_no_imu_data_msg_latch);
        ...
        if(m_no_imu_data_msg_latch != 0) {
            rclcpp::Duration imu_normal_duration = last_imu_time_ - no_imu_detected_time_;
            RCLCPP_WARN(this->get_logger(), "IMU data received ok after %.2f seconds.", imu_normal_duration.seconds());            
        }
