...
OdomMonitor::OdomMonitor() : Node("odom_monitor"), last_odom_time_(std::chrono::steady_clock::now()), last_imu_time_(std::chrono::steady_clock::now()) {
...
void OdomMonitor::odom_callback(const nav_msgs::msg::Odometry::SharedPtr msg) {
    ...
    std::chrono::duration<double> elapsed_sec = now-last_odom_time_;
...
void OdomMonitor::imu_callback(const sensor_msgs::msg::Imu::SharedPtr msg) {
    auto now = std::chrono::steady_clock::now();
    std::chrono::duration<double> elapsed_sec = now-last_imu_time_;
    double dt = elapsed_sec.count();
...
void OdomMonitor::data_timer_callback() {
    auto now = std::chrono::steady_clock::now();
    std::chrono::duration<double> odom_duration = now-last_odom_time_;
    double odom_time_diff = odom_duration.count();
     std::chrono::duration<double> imu_duration = now-last_imu_time_;
    double imu_time_diff = imu_duration.count();

    if (odom_time_diff > 0.05) {
        if(m_no_odom_data_msg_latch == 0){
            no_odom_detected_time_ = last_odom_time_;
        }

        if(m_no_odom_data_msg_latch < 5) {
            RCLCPP_WARN(this->get_logger(), "No odometry data received for %.2f seconds.[%d]", odom_time_diff, m_no_odom_data_msg_latch);
            m_no_odom_data_msg_latch = 205;
        }else {
            m_no_odom_data_msg_latch--;
        }
    } else {
        if(m_no_odom_data_msg_latch != 0){
            std::chrono::duration<double> odom_normal_duration = last_odom_time_-no_odom_detected_time_;
            double odom_normal_duration_sec = odom_normal_duration.count();
            RCLCPP_WARN(this->get_logger(), "odometry data received ok after %.2f seconds.", odom_normal_duration_sec);            
        }
        m_no_odom_data_msg_latch = 0;
    }

    if (imu_time_diff > 0.05) {
        if(m_no_imu_data_msg_latch == 0){
            no_imu_detected_time_ = last_imu_time_;
        }

        if(m_no_imu_data_msg_latch < 5) {
            RCLCPP_WARN(this->get_logger(), "No IMU data received for %.2f seconds.[%d]", imu_time_diff, m_no_imu_data_msg_latch);
            m_no_imu_data_msg_latch = 205;
        } else {
            m_no_imu_data_msg_latch--;
        }
    } else {
        if(m_no_imu_data_msg_latch != 0) {
            std::chrono::duration<double> imu_normal_duration = last_imu_time_-no_imu_detected_time_;
            double imu_normal_duration_sec = imu_normal_duration.count();
            RCLCPP_WARN(this->get_logger(), "IMU data received ok after %.2f seconds.", imu_normal_duration_sec);            
        }
        m_no_imu_data_msg_latch = 0;
    }
}
