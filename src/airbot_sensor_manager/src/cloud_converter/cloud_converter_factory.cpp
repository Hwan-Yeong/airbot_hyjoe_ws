...
{
    using ConverterCreator = std::function<std::shared_ptr<CloudConverterStrategy>()>;
    static const std::unordered_map<std::string, ConverterCreator> factory = {
        {"tof_mono",        [&]() { return std::make_shared<TofMonoCloudConverter>(node_ptr, config); }},
        {"tof_multi",       [&]() { return nullptr; }},
        {"tof_multi_left",  [&]() { return std::make_shared<TofMultiLeftCloudConverter>(node_ptr, config); }},
        {"tof_multi_right", [&]() { return std::make_shared<TofMultiRightCloudConverter>(node_ptr, config); }},
        {"camera",          [&]() { return std::make_shared<CameraCloudConverter>(node_ptr, config); }},
        {"bottom_ir",       [&]() { return std::make_shared<BottomIrCloudConverter>(node_ptr, config); }},
        {"collision_front", [&]() { return std::make_shared<CollisionCloudConverter>(node_ptr, config); }},
        {"collision_rear",  [&]() { return std::make_shared<CollisionCloudConverter>(node_ptr, config); }},
        {"empty",           [&]() { return std::make_shared<EmptyCloudConverter>(node_ptr, config); }}
    };
