...
public:
  ...
  void handleBattery(const robot_custom_msgs::msg::BatteryStatus::SharedPtr msg);
  void handleBatterySleepTimer();
  void releaseError();
  ...
  std::shared_ptr<Following> following;
  std::shared_ptr<ManualControl> manual_control;
  ...
  rclcpp::TimerBase::SharedPtr batterysleep_send_timer_;

  
  std::vector<robot_custom_msgs::msg::ErrorList> error_list;
  std::vector<robot_custom_msgs::msg::ErrorList> manage_error_list;
  
  ROBOT_STATE current_state;
  state_cmd pre_cmds;

  double current_time = 0.0;
  double error_start_time = 0.0;
  bool battery_error_init_time = false;
  bool battery_percent_cnt_init_time = false;
