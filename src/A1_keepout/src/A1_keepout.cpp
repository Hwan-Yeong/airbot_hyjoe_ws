// 2025-01-06 clabil v1.2
#include "A1_keepout/A1_keepout.hpp"
#include <ament_index_cpp/get_package_share_directory.hpp>

#define PRINT_INFO false

NavKeepoutService::NavKeepoutService() : Node("A1_keepout_node") {
  ...
  json_file_path_ =
      this->get_parameter("json_file_path").get_value<std::string>();
  publish_topic_name_ =
      this->get_parameter("publish_topic_name").get_value<std::string>();
  this->get_parameter("obstacle_point_resolution", obs_point_resolution_);

  RCLCPP_INFO(this->get_logger(), "Keepout json file : %s",
              json_file_path_.c_str());
  RCLCPP_INFO(this->get_logger(), "Reqeust time(ms) : %d", reqeust_time_ms_);
  RCLCPP_INFO(this->get_logger(), "Publish topic name : %s",
              publish_topic_name_.c_str());
  RCLCPP_INFO(this->get_logger(), "Obstacle point resolution : %f",
              obs_point_resolution_);
  RCLCPP_INFO(this->get_logger(), "param read complete!");
  ...
  block_line_sub_ =
      this->create_subscription<robot_custom_msgs::msg::BlockAreaList>(
          "/block_walls", qos_profile,
          std::bind(&NavKeepoutService::block_wall_callback, this,
                    std::placeholders::_1));

  block_area_sub_ =
      this->create_subscription<robot_custom_msgs::msg::BlockAreaList>(
          "/block_areas", qos_profile,
          std::bind(&NavKeepoutService::block_area_callback, this,
                    std::placeholders::_1));

  // keepout_zone_pub_ =
  // this->create_publisher<sensor_msgs::msg::PointCloud2>("/A1_keepout/point_cloud",
  // 10);
  keepout_zone_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(
      publish_topic_name_, 10);
  ...
  RCLCPP_INFO(this->get_logger(), "A1 keepout service init finished!");
}

// 경로가 존재하지 않으면 디렉토리 생성
void NavKeepoutService::create_directory_if_not_exists(
...
void NavKeepoutService::load_keepout_zones_from_json(
    const std::string &file_path, json &json_data) {
...
#if PRINT_INFO
  RCLCPP_INFO(this->get_logger(), "JSON Data: %s", json_data.dump().c_str());
#endif
}

void NavKeepoutService::save_json_to_file(const json &json_data,
...
void NavKeepoutService::block_area_callback(
    const robot_custom_msgs::msg::BlockAreaList::SharedPtr msg) {
  ...
  for (const auto &vwall : msg->block_area_list) {
    ...
    for (const auto &position : vwall.robot_path) {
      keepout_zones["keepout_zones"][zone_index]["coordinates"][index]["x"] =
          (double)position.x;
      keepout_zones["keepout_zones"][zone_index]["coordinates"][index]["y"] =
          (double)position.y;
      index++;
    }
    zone_index++;
...
void NavKeepoutService::block_wall_callback(
    const robot_custom_msgs::msg::BlockAreaList::SharedPtr msg) {
...
void NavKeepoutService::run() {
  auto timer_ = this->create_wall_timer(
      ...
      std::bind(&NavKeepoutService::timer_callback, this));
...
std::vector<point> NavKeepoutService::generate_pointcloud_by_coordinates(
    const std::vector<point> &origin) {
  std::vector<point> result;
  point start;
  point end;
  point generated;

  if (origin.size() == 4) { // virtual-area
  ...
  } else if (origin.size() == 2) { // virtual-line
    ...
    throw std::invalid_argument("Input should contain 2 or 4 points.");
  }
...
double NavKeepoutService::get_distance_two_points(point robot, point target) {
  return std::sqrt(std::pow(target.x - robot.x, 2) +
                   std::pow(target.y - robot.y, 2));
}
...
#if PRINT_INFO
  std::cout << "Published " << points.size()
            << " points to the /point_cloud topic." << std::endl;
#endif
}
