...
class ErrorManagerNode : public rclcpp::Node {
 ...
 private:
  ...
  void errorCallback(const std::string& error_code,
  ...
  void resolveErrorLists(std::string code);
  void allErrorResolved();
  void printErrorList();
  // void LidarErrorResolve();

  YAML::Node config{};
  std::vector<tErrorList> error_list_;
  std::vector<std::string> pending_resolves_;
  ...
  rclcpp::Publisher<robot_custom_msgs::msg::ErrorListArray>::SharedPtr
      error_list_pub_;
