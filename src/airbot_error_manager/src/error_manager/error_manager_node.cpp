...
ErrorManagerNode::ErrorManagerNode()
  ...
  rcutils_ret_t set_logger = rcutils_logging_set_logger_level(
      this->get_logger().get_name(),
      static_cast<RCUTILS_LOG_SEVERITY>(log_level));
  if (set_logger != RCUTILS_RET_OK) {
    RCLCPP_ERROR(this->get_logger(),
                 "Failed to set logger level (error code: %d)", set_logger);
  }
  ...
  try {
    ...
    // RCLCPP_INFO(this->get_logger(), "package_share_directory: %s, file: %s",
    //             package_share_directory.c_str(), error_list.c_str());
    std::string full_path =
        package_share_directory + "/" + "config" + "/" + error_list;
    this->config =
        YAML::LoadFile(full_path)["airbot_error_manager"]["error_list"];
    // RCLCPP_INFO(this->get_logger(), "YAML Config: \n%s",
    //             YAML::Dump(this->config).c_str());
  } catch (const std::exception& e) {
    RCLCPP_ERROR(this->get_logger(),
                 "std::exception Failed to load config file: %s", e.what());
    std::string fallback_path =
        "/home/airbot/airbot_ws/install/airbot_error_manager/share/"
        "airbot_error_manager/config/" +
        error_list;
    this->config =
        YAML::LoadFile(fallback_path)["airbot_error_manager"]["error_list"];
  }
  ...
  // 도킹불가 에러 (from state_manager)
  erase_after_pub_error_codes_.insert("S08");
  // 좌측구동모터 구속 에러 (from mcu)
  erase_after_pub_error_codes_.insert("E04");
  // 우측구동모터 구속 에러 (from mcu)
  erase_after_pub_error_codes_.insert("E04-1");
  // 전면 라이다 센서 오염 (from lidar)
  erase_after_pub_error_codes_.insert("E05");
  // 후면 라이다 센서 오염 (from mcu)
  erase_after_pub_error_codes_.insert("E06");
  // 스테이션 위치 확인 불가 (from state_manager)
  erase_after_pub_error_codes_.insert("E07");
  // 충전불가 (from error_monitor)
  // erase_after_pub_error_codes_.insert("E08");
  // 카메라 & AI 통신 불량(비복귀에러)
  // erase_after_pub_error_codes_.insert("F09-2");

  error_list_pub_ =
      this->create_publisher<robot_custom_msgs::msg::ErrorListArray>(
          "/error_list", 10);
  robot_state_sub_ =
      this->create_subscription<robot_custom_msgs::msg::RobotState>(
          "/state_datas", qos_state_profile,
          std::bind(&ErrorManagerNode::robotStateCallback, this,
                    std::placeholders::_1));

  pub_timer_ = this->create_wall_timer(
      publish_rate_ms, std::bind(&ErrorManagerNode::publishErrorList, this));
...
void ErrorManagerNode::initSubscribers(const YAML::Node& config) {
  auto pnode =
      std::static_pointer_cast<ErrorManagerNode>(this->shared_from_this());
  if (!pnode) {
  ...
  for (const auto& error_category : config) {
    ...
    for (const auto& error : category_value) {
      ...
      error_code_descriptions_[error_code] =
      ...
      auto callback = [this, pnode,
                       error_code](std_msgs::msg::Bool::SharedPtr msg) {
        errorCallback(error_code, msg);
      };
      auto subs = this->create_subscription<std_msgs::msg::Bool>(sub_topic, 10,
                                                                 callback);
...
void ErrorManagerNode::errorCallback(const std::string& error_code,
...
void ErrorManagerNode::robotStateCallback(
    const robot_custom_msgs::msg::RobotState::SharedPtr msg) {
...
void ErrorManagerNode::publishErrorList() {
  ...
  if (!isResolvedAllError && prev_robot_state == 9 &&
      cur_robot_state != 9) {  //ROBOT_STATE::ERROR
    RCLCPP_INFO(this->get_logger(),
                "Realesed all error in error_lists_ [Num of errors: %zu]",
                error_list_.size());
    allErrorResolved();
  ...
  for (auto it = error_list_.begin(); it != error_list_.end();) {
    auto& error = *it;
    // 퍼블리시 필요 없고, 해제 상태인데 카운트도 넘었으면 삭제
    ...
    // pub_cnt 만큼 퍼블리싱 했으면 발행 정지.
    if (error.error.count > pub_cnt) {
      // 에러 발생 후 에러 리스트에서 관리하지 않을 에러 제거
      if (erase_after_pub_error_codes_.count(error.error.error_code)) {
        it = error_list_.erase(it);
        continue;
      } else {
...
void ErrorManagerNode::addError(const std::string& error_code) {
  ...
  // error_list_에 이미 동일한 error가 존재하면 무시.
  if (it != error_list_.end()) {
    return;
  }

  if (error_list_.size() >
      static_cast<size_t>(error_list_size)) {  // error_list_ 최대 크기 제한
...
void ErrorManagerNode::resolveErrorLists(std::string code) {
  ...
  if (it != error_list_.end()) {  // 해제는 발생했던 에러에만 적용
    // 과거에 발생한 적이 있을 때만 해제 메시지 퍼블리싱 활성화
    if (it->has_occurred_before && it->error.error_occurred) {
  ...
  for (auto& error : error_list_) {
    if (error.error.error_code.find("F13") != std::string::npos ||
      ...
      if (error.error.error_occurred) {
        // 에러가 활성 상태인 경우
        if (lidar_error_occured_times.find(error.error.error_code) ==
            lidar_error_occured_times.end()) {
          // 에러코드와 타임 추가 (타이머 시작)
          lidar_error_occured_times[error.error.error_code] =
              clock.now().seconds();
          RCLCPP_INFO(this->get_logger(),
                      "Error %s detected. Starting 5s auto resolve timer.",
          ...
          if ((clock.now().seconds() - trigger_time) >= 5.0) {
            RCLCPP_INFO(this->get_logger(),
                        "auto resolve error %s after 5 seconds.",
                        error.error.error_code.c_str());
            error.error.error_occurred = false;
            error.should_publish = true;
