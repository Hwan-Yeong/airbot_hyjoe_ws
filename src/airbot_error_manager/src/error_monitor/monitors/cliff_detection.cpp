...
void CliffDetectionErrorMonitor::printParams() const {
    if (!node_ptr_) return;
    RCLCPP_INFO(node_ptr_->get_logger(), "[%s] duration_sec: %.1f, accum_dist_th_m: %.1f, rate: %d",
        paramNamespace().c_str(), params.duration_sec, params.accum_dist_th, params.monitoring_rate_ms);
}
...
void CliffDetectionErrorMonitor::timerCallback()
{
    std::tuple<robot_custom_msgs::msg::BottomIrData, nav_msgs::msg::Odometry, robot_custom_msgs::msg::RobotState> input;
    {
        ...
        if (!ir.is_updated || !odom.is_updated || !rs.is_updated) return;
        input = std::make_tuple(ir.data, odom.data, rs.data);
    }
