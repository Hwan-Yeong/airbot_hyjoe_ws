...
void FallDownErrorMonitor::printParams() const {
  ...
  RCLCPP_INFO(
      ...
      params.drop_ir_adc_th, params.drop_ir_cnt_min, params.imu_roll_th,
      params.imu_pitch_th);
}

void FallDownErrorMonitor::startMonitor(
 ...
 *    1000ms
 * - 발생 조건:
 *    아래 두 조건 동시 성립 시 즉시 발생
 *      1) (IR) 하단 IR 센서 중 adc값 900 미만인 센서가 4개 이상 동시 감지
 *      2) (IMU) 절대값 기준 Roll 60도 이상 또는 Pitch 60도 이상 감지
 * - 해제 조건:
 *    1) 6개 IR 센서중 들림 감지 상태의 센서 개수가 4개 미만으로 줄어든 경우
 *    2) Pitch/Roll이 모두 60도 미만으로 돌아온 경우
 */
void FallDownErrorMonitor::timerCallback() {
  ...
  if (!ir.is_updated || !imu.is_updated)
    return;
  ...
  if (abs(deg_roll - baseline_roll_deg) >= 10.0 ||
    ...
    RCLCPP_INFO(node_ptr_->get_logger(),
                ...
                paramNamespace().c_str(), baseline_pitch_deg, baseline_roll_deg,
                deg_pitch, deg_roll, duration);
    baseline_pitch_deg = deg_pitch;
    baseline_roll_deg = deg_roll;
    baseline_time = clock.now().seconds();
  }
  ...
  if (is_imu_out_of_range &&
      is_ir_low_adc) {  // 전도가 일어남, 데이터 값에 따른 결정
    if (!prev_status) {
      ...
      RCLCPP_INFO(node_ptr_->get_logger(),
                  ...
                  ir.data.adc_br, deg_pitch, deg_roll);
      prev_status = true;
    }
    ...
    if (prev_status) {
      // [250407] hyjoe : 전도 에러 발생 한적이 있었던 경우, 해제시 1번만 낙하IR상태
      // roll, pitch 정보 1번만 로깅
      RCLCPP_INFO(node_ptr_->get_logger(),
...
void FallDownErrorMonitor::get_rpy_from_quaternion(
