...
void LiftErrorMonitor::printParams() const {
    if (!node_ptr_) return;
    RCLCPP_INFO(node_ptr_->get_logger(), "[%s] drop_ir_adc: %d, ir_cnt_min: %d, imu_z_acc_low: %.1f, imu_z_acc_high: %.1f, rate: %d",
        paramNamespace().c_str(), params.drop_ir_adc_th, params.drop_ir_cnt_min, params.imu_z_acc_low_th, params.imu_z_acc_hight_th, params.monitoring_rate_ms);
}
