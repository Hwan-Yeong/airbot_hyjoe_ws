...
namespace explore {
    ...
    void ThetaStar::set_start_and_goal(
    ...
    bool ThetaStar::generate_path(std::vector<coordsW> &raw_path) {
        reset_containers();
        add_to_nodes_data(index_generated_);
        double src_g_cost = get_traversal_cost(src_.x, src_.y), src_h_cost = get_h_cost(src_.x, src_.y);
        ...
        add_index(src_.x, src_.y, &nodes_data_[index_generated_]);
        tree_node *curr_data = &nodes_data_[index_generated_];
        index_generated_++;
        nodes_opened = 0;
        while (!queue_.empty()) {
            nodes_opened++;
            if (is_goal(*curr_data)) {
                break;
            }
            reset_parent(curr_data);
            set_neighbors(curr_data);
    ...
    void ThetaStar::reset_parent(tree_node *curr_data) {
        ...
        if (los_check(curr_data->x, curr_data->y, maybe_par->x, maybe_par->y, los_cost)) {
            g_cost = maybe_par->g +
                     get_euclidean_cost(curr_data->x, curr_data->y, maybe_par->x, maybe_par->y) + los_cost;
    ...
    void ThetaStar::set_neighbors(const tree_node *curr_data) {
        int mx, my;
        ...
        for (int i = 0; i < how_many_corners_; i++) {
            ...
            g_cost = curr_data->g + get_euclidean_cost(curr_data->x, curr_data->y, mx, my) +
                     get_traversal_cost(mx, my);
            m_id = get_index(mx, my);
            if (m_id == nullptr) {
                add_to_nodes_data(index_generated_);
                m_id = &nodes_data_[index_generated_];
                add_index(mx, my, m_id);
                index_generated_++;
            }
            exp_node = m_id;
            h_cost = get_h_cost(mx, my);
    ...
    bool ThetaStar::los_check(
        ...
        if (dx >= dy) {
            while (cx != x1) {
                f += dy;
                if (f >= dx) {
                    if (!is_safe(cx + u_x, cy + u_y, sl_cost)) {
                        return false;
                    }
                    cy += sy;
                    f -= dx;
                }
                if (f != 0 && !is_safe(cx + u_x, cy + u_y, sl_cost)) {
                    return false;
                }
                if (dy == 0 && !is_safe(cx + u_x, cy, sl_cost) && !is_safe(cx + u_x, cy - 1, sl_cost)) {
                    return false;
                }
                cx += sx;
            }
            ...
            while (cy != y1) {
                ...
                if (dx == 0 && !is_safe(cx, cy + u_y, sl_cost) && !is_safe(cx - 1, cy + u_y, sl_cost)) {
                    return false;
                }
                cy += sy;
            }
    ...
    void ThetaStar::reset_containers() {
        index_generated_ = 0;
        ...
        if (((last_size_x != curr_size_x) || (last_size_y != curr_size_y)) &&
            ...
            initialize_pose(curr_size_y * curr_size_x - last_size_y * last_size_x);
    ...
    void ThetaStar::initialize_pose(int size_inc) {
